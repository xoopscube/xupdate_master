...
define('XUPDATEMASTER_FRAME_PERFORM_SUCCESS', 1);
define('XUPDATEMASTER_FRAME_PERFORM_FAIL', 2);
define('XUPDATEMASTER_FRAME_INIT_SUCCESS', 3);

define('XUPDATEMASTER_FRAME_VIEW_NONE','none');
define('XUPDATEMASTER_FRAME_VIEW_SUCCESS','success');
define('XUPDATEMASTER_FRAME_VIEW_ERROR','error');
define('XUPDATEMASTER_FRAME_VIEW_INDEX','index');
define('XUPDATEMASTER_FRAME_VIEW_INPUT','input');
define('XUPDATEMASTER_FRAME_VIEW_PREVIEW','preview');
define('XUPDATEMASTER_FRAME_VIEW_CANCEL','cancel');

/**
 * Xupdatemaster_Module
**/
class Xupdatemaster_Module extends Legacy_ModuleAdapter
{
	public /*** string ***/ $mActionName = null;

	public /*** Xupdatemaster_AbstractAction ***/ $mAction = null;

	public /*** bool ***/ $mAdminFlag = false;

	public /*** Xupdatemaster_AssetManager ***/ $mAssetManager = null;
	...
	protected /*** Enum[] ***/ $_mAllowViewNames = array(
		XUPDATEMASTER_FRAME_VIEW_NONE,
		XUPDATEMASTER_FRAME_VIEW_SUCCESS,
		XUPDATEMASTER_FRAME_VIEW_ERROR,
		XUPDATEMASTER_FRAME_VIEW_INDEX,
		XUPDATEMASTER_FRAME_VIEW_INPUT,
		XUPDATEMASTER_FRAME_VIEW_PREVIEW,
		XUPDATEMASTER_FRAME_VIEW_CANCEL
	);
	...
	**/
	public function startup()
	{
		...
		XCube_DelegateUtils::call('Module.xupdatemaster.Global.Event.GetAssetManager',new XCube_Ref($this->mAssetManager),$this->mXoopsModule->get('dirname'));
	...
	**/
	private function _getDefaultActionName()
	{
		...
		$dataname = isset($dataname) ? $dataname : 'Store';
	...
	public function getRenderSystemName()
	{
		...
		if(!defined('XUPDATEMASTER_ADMIN_RENDER_REGISTED'))
		{
			...
			$root->overrideSiteConfig(
				array(
					'RenderSystems' => array(
						'Xupdatemaster_AdminRenderSystem' => 'Xupdatemaster_AdminRenderSystem'
					),
					'Xupdatemaster_AdminRenderSystem' => array(
						'root' => XUPDATEMASTER_TRUST_PATH,
						'path' => '/admin/class/XupdatemasterAdminRenderSystem.class.php',
						'class' => 'Xupdatemaster_AdminRenderSystem'
					)
				)
			);
		}
	...
	private function _createAction()
	{
		...
		switch(true)
		{
			case file_exists(
				...
				break;
			case file_exists(
				$path = XUPDATEMASTER_TRUST_PATH . '/' . $fileName
			):
				break;
			default:
				return false;
		...
		$className = 'Xupdatemaster_' . ($this->mAdminFlag ? 'Admin_' : '')
			. ucfirst($this->mActionName) . 'Action';
		if(class_exists($className))
		{
			$this->mAction = new $className();
		}
		if(!$this->mAction instanceof Xupdatemaster_AbstractAction)
		{
			return false;
		}
	...
	**/
	...
	{
		/**
		 * Module.xupdatemaster.Global.Event.Exception.ActionNotFound
		 * 
		 * @param	string	$dirname
		 * 
		 * @return	void
		**/
		XCube_DelegateUtils::call('Module.xupdatemaster.Global.Event.Exception.ActionNotFound',$this->mAssetManager->mDirname);
		/**
	...
	**/
	...
	{
		/**
		 * Module.xupdatemaster.Global.Event.Exception.Preparation
		 * 
		 * @param	string	$dirname
		 * 
		 * @return	void
		**/
		XCube_DelegateUtils::call('Module.xupdatemaster.Global.Event.Exception.Preparation',$this->mAssetManager->mDirname);
		/**
